...
import { Target, ChevronDown, Activity as ActivityIcon, Search } from "lucide-react";
import { getDisplayUrl } from "@/lib/utils";

type Activity = any; // We can use any here to save time on complex types, since it's just passing data
type Stage = any;
type Program = any;

interface ActivitiesClientProps {
  programs: Program[];
  standaloneActivities: Activity[];
}

export default function ActivitiesClient({ programs, standaloneActivities }: ActivitiesClientProps) {
  const [searchTerm, setSearchTerm] = useState("");
  const now = new Date();

  const filteredData = useMemo(() => {
    if (!searchTerm.trim()) {
      return { programs, standaloneActivities };
    }

    const lowerQuery = searchTerm.toLowerCase();

    // Filter standalone activities
    const filteredStandalone = standaloneActivities.filter((activity: Activity) => 
      activity.title.toLowerCase().includes(lowerQuery) || 
      (activity.description && activity.description.toLowerCase().includes(lowerQuery))
    );

    // Filter programs and their nested stages/activities
    const filteredPrograms = programs.map((program: Program) => {
      const isProgramMatch = program.title.toLowerCase().includes(lowerQuery);

      const filteredStages = program.stages.map((stage: Stage) => {
        const isStageMatch = stage.title.toLowerCase().includes(lowerQuery);

        const filteredActivities = stage.activities.filter((activity: Activity) => 
          isProgramMatch || isStageMatch || activity.title.toLowerCase().includes(lowerQuery) || 
          (activity.description && activity.description.toLowerCase().includes(lowerQuery))
        );
        ...
        return {
          ...
          // Keep stage if it matches, or if any of its activities match, or if program matches
          _keep: isProgramMatch || isStageMatch || filteredActivities.length > 0
        };
      }).filter((stage: any) => stage._keep);

      return {
        ...program,
        stages: filteredStages,
        // Keep program if it matches, or if any stage is kept
        _keep: isProgramMatch || filteredStages.length > 0
      };
    }).filter((program: any) => program._keep);

    return {
      programs: filteredPrograms,
      standaloneActivities: filteredStandalone
    };
  }, [programs, standaloneActivities, searchTerm]);

  const { programs: displayPrograms, standaloneActivities: displayStandalone } = filteredData;
  ...
  return (
    <div className="flex-1 w-full space-y-8">
      ...
      <div className="max-w-2xl mx-auto relative">
        ...
        <input
          type="text"
          className="block w-full pl-11 pr-4 py-4 border border-gray-200 rounded-2xl leading-5 bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm shadow-sm transition-shadow"
          placeholder="Tìm kiếm theo tên chương trình, chặng hoặc hoạt động..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
        />
      </div>

      {displayPrograms.length === 0 && displayStandalone.length === 0 && (
        <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
          <p className="text-gray-500 font-medium">Không tìm thấy hoạt động nào phù hợp với tìm kiếm của bạn.</p>
        </div>
      )}
        ...
        {displayPrograms.map((program: Program) => (
          <div key={program.id} className="relative">
            ...
            <div className="space-y-4 pl-[19px]">
              {program.stages.map((stage: Stage) => (
                <details key={stage.id} className="group" open>
                  ...
                  <div className="border-l-2 border-emerald-500 border-b border-r border-emerald-100 rounded-b-xl bg-white p-4 space-y-3 relative -left-[1px]">
                    {stage.activities.length === 0 ? (
                      ...
                      stage.activities.map((activity: Activity) => (
      ...
      {displayStandalone.length > 0 && (
        <div className="mt-16">
          ...
          <div className="grid gap-4 pl-[19px]">
            {displayStandalone.map((activity: Activity) => (
